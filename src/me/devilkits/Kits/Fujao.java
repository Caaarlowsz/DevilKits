...
public class Fujao implements Listener, CommandExecutor
{
    ...
    public void stomperApple(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().getItemInHand().getType() == Material.GHAST_TEAR && ArrayL.fujao.contains(event.getPlayer().getName())) {
            ...
            Bukkit.getServer().getScheduler().scheduleAsyncDelayedTask((Plugin)Fujao.plugin, Fujao1.FujaoFuncao(this, p), 500L);
        }
    }
