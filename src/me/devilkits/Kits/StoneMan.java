...
public class StoneMan implements Listener, CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (label.equalsIgnoreCase("stoneman")) {
            if (!Main.usandokit.contains(p.getName()) && p.hasPermission("kit.stoneman")) {
                p.sendMessage("§7Voce pegou o Kit : §6StoneMan");
                p.playSound(p.getLocation(), Sound.NOTE_PLING, 4.0f, 4.0f);
                Main.usandokit.add(p.getName());
                Main.stoneman.add(sender.getName());
    ...
    public void onPlayerCamel(final PlayerMoveEvent e) {
        ...
        if ((e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.STONE || e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.COBBLESTONE) && Main.stoneman.contains(p.getName())) {
