			...
			if (p.hasPermission("DevilKits.kick") || p.isOp()) {
				if (args.length == 0) {
					...
					if (targetPlayer == null) {
						...
						Bukkit.broadcastMessage(ChatColor.RED + targetPlayer.getDisplayName() + "§7 foi kickado por §c"
								+ p.getDisplayName() + ".");
					}
				} else if (args.length > 1) {
					final Player targetPlayer = p.getServer().getPlayer(args[0]);
					if (targetPlayer == null) {
						...
						targetPlayer.kickPlayer(
								"§7Voce foi kickado pelo jogador §c" + p.getDisplayName() + " §7por > " + message);
						Bukkit.broadcastMessage(ChatColor.RED + targetPlayer.getDisplayName() + " §7Foi kickado por §c"
								+ p.getDisplayName() + " §7por " + message);
					}
				}
			} else {
