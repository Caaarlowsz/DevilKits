...
public class CombatLog implements Listener
{
    ...
    public void onMe2(final PlayerCommandPreprocessEvent event) {
        ...
        if (CombatLog.Sair.contains(p.getName()) && (event.getMessage().toLowerCase().startsWith("/spawn") || event.getMessage().toLowerCase().startsWith("/fps") || event.getMessage().toLowerCase().startsWith("/hg") || event.getMessage().toLowerCase().startsWith("/set") || event.getMessage().toLowerCase().startsWith("/lava") || event.getMessage().toLowerCase().startsWith("/textura") || event.getMessage().toLowerCase().startsWith("/suicide") || event.getMessage().toLowerCase().startsWith("/morrer") || event.getMessage().toLowerCase().startsWith("/hg") || event.getMessage().toLowerCase().startsWith("/knock") || event.getMessage().toLowerCase().startsWith("/mlg") || event.getMessage().toLowerCase().startsWith("/1v1"))) {
